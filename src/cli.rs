...
impl Cli {
  ...
  pub fn run(&self) -> Result<()> {
    match &self.command {
      ...
      FmtCommands::Wrap(input) => {
        println!("{}", wrap(input.text.as_deref(), input.width));
      },
      FmtCommands::Text(input) => {
        println!("{}", text(
          input.text.as_deref(),
          Some(input.width),
          Some(input.frame),
